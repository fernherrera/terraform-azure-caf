...
locals {
  ...
  backup_storage_account = can(var.backup.storage_account_key) ? var.storage_accounts[var.backup.storage_account_key] : null
  backup_sas_url         = can(var.backup.storage_account_key) ? "${local.backup_storage_account.primary_blob_endpoint}${local.backup_storage_account.containers[var.backup.container_key].name}${data.azurerm_storage_account_blob_container_sas.backup[0].sas}" : null

  logs_storage_account = can(var.logs.storage_account_key) ? var.storage_accounts[var.logs.storage_account_key] : null
  logs_sas_url         = can(var.logs.storage_account_key) ? "${local.logs_storage_account.primary_blob_endpoint}${local.logs_storage_account.containers[var.logs.container_key].name}${data.azurerm_storage_account_blob_container_sas.logs[0].sas}" : null

  http_logs_storage_account = can(var.logs.http_logs.storage_account_key) ? var.storage_accounts[var.logs.http_logs.storage_account_key] : null
  http_logs_sas_url         = can(var.logs.http_logs.storage_account_key) ? "${local.http_logs_storage_account.primary_blob_endpoint}${local.http_logs_storage_account.containers[var.logs.http_logs.container_key].name}${data.azurerm_storage_account_blob_container_sas.http_logs[0].sas}" : null
}
        ...
        content {
          ...
          dynamic "azure_blob_storage" {
            ...
            content {
              ...
              sas_url           = try(var.logs.application_logs.azure_blob_storage.sas_url, local.logs_sas_url)
            }
          }
        }
      }
  ...
  dynamic "site_config" {
    ...
    content {
      ...
      dynamic "application_stack" {
        ...
        content {
          docker_image        = try(var.site_config.application_stack.docker_image, null)
          docker_image_tag    = try(var.site_config.application_stack.docker_image_tag, null)
...
resource "azurerm_app_service_source_control" "scm" {
  ...
  dynamic "github_action_configuration" {
    for_each = try(var.source_control.github_action_configuration, {}) != {} ? [1] : []
    ...
    content {
      ...
      dynamic "code_configuration" {
        for_each = try(var.source_control.github_action_configuration.code_configuration, {}) != {} ? [1] : []
      ...
      dynamic "container_configuration" {
        for_each = try(var.source_control.github_action_configuration.container_configuration, {}) != {} ? [1] : []
